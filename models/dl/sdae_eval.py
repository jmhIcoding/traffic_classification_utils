...
from data_utils import LoadDataNoDefCW100,LoadDataNoDefCW,LoadDataWalkieTalkieCW,LoadDataApp,LoadDataApp_crossversion
...
def test_sdae(X_train,y_train,X_valid,y_valid,X_test,y_test):
    ...
    model.load_model()
    score = model.evaluate(X_test=X_test,y_test=y_test)
    print('sdae Test on test dataset accuracy :{0}'.format(score))
if __name__ == '__main__':
    X_train, y_train, X_valid, y_valid, X_test, y_test =  LoadDataApp_crossversion()#LoadDataWalkieTalkieCW() #LoadDataNoDefCW()
